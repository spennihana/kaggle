...
public class ParallelCsvRead {
  ...
  int _strlen;
  int _sz;
  public ParallelCsvRead(String path, int strlen, int sz) {
  ...
  public HashMap<BufferedString, BufferedString>[] parse_bin() {
  ...
  private HashMap<BufferedString, BufferedString>[] parse_bits() {
    long start = System.currentTimeMillis();
    _pbits = new ParseBitsTask[_rtasks.length];
    ArrayList<ParseBitsTask> ptasks = new ArrayList<>();
    HashMap<BufferedString,BufferedString> maps[] = new HashMap[_pbits.length];
  ...
  public static class ParseBitsTask extends RecursiveAction {
    ...
    int _strLen;
    int _sz;
    final HashMap<BufferedString, BufferedString> _rows;
    ...
    @Override protected void compute() {
      ...
      while(pos < _in.length) {
        ...
        BufferedString bs = new BufferedString(_in,pos,sz);
        pos+=sz;
        pos+=_strLen-sz;
        _rows.put(bs,new BufferedString(_in,pos,4*300));
        pos+=4*300;
  ...
  public static void main(String[] args) {
    ...
    ParallelCsvRead r2 = new ParallelCsvRead("./lib/w2vec_models/glove.bin",1005,2);
    HashMap<BufferedString, BufferedString>[] glove = r2.parse_bin();
    r2._pbits = null; r2._rtasks=null;
    ParallelCsvRead r = new ParallelCsvRead("./lib/w2vec_models/googl.bin",208,1);
    HashMap<BufferedString, BufferedString>[] googl = r.parse_bin();
